  ...
  return (
    <div className="relative flex items-center">
        ...
        <div className="absolute top-full mt-2 w-full bg-white border border-gray-300 rounded-lg shadow-lg z-50">
          <ul className="max-h-60 overflow-y-auto">
            {searchResults.map((product) => (
              <li key={product.id} className="p-4 border-b last:border-b-0">
                <Link to={`/shop-detail/${product.ProductID}`} className="flex justify-between items-center">
                  <div>
      ...
      {searchResults.length === 0 && query && (
        <p className="text-gray-500 absolute top-full mt-2">Không tìm thấy sản phẩm nào trùng khớp.</p>
      )}
    </div>
  );
};
